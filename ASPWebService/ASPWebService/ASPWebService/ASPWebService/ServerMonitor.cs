...
namespace ServerManagement
{
    public class ServerMonitor
    {
        public static ServerInfo CollectServerInfo()
        {
            ...
            PerformanceCounter hyperVCPUUsageCounter = new PerformanceCounter
               ("Hyper-V Hypervisor Logical Processor", "% Total Run Time", "_Total");
            hyperVCPUUsageCounter.NextValue();

            List<int> averageReadings = new List<int>();
            //read 5 times the free CPU in order to filter out spikes and have an average value
            for (int i = 0; i < 5; i++)
            {
                List<int> cpuReading = new List<int>();
                ...
                {
                    cpuReading.Add((int)((100 - hyperVCPUUsageCounter.NextValue()) / 100 * serverInfo.TotalCPU));
                }
                if (averageReadings.Count == 0)
                {
                    for (int j = 0; j < cpuReading.Count; j++)
                    {
                        averageReadings.Add(cpuReading.ElementAt(j));
                    }
                }
                else
                {
                    for (int j = 0; j < cpuReading.Count; j++)
                    {
                        int value = averageReadings.ElementAt(j);
                        averageReadings.RemoveAt(j);
                        averageReadings.Insert(j, value + cpuReading.ElementAt(j));
                    }
                }
            }
            int average_value = 0;
            for (int i = 0; i < averageReadings.Count; i++)
            {
                average_value += averageReadings.ElementAt(i);
            }
            average_value /= averageReadings.Count * 5;
            for (int i = 0; i < averageReadings.Count; i++)
            {
                averageReadings.RemoveAt(i);
                averageReadings.Insert(i, average_value);
            }

           


            serverInfo.FreeCPU = averageReadings;
